...
const MOCK_TOURNAMENTS: Record<string, TournamentData> = {
  'autumn-blitz-2025': {
...
function buildMockArticle(td: TournamentData): GeneratedArticle {
  ...
  const body = `${winner.title} ${winner.playerName} claimed the ${td.name} title with a commanding performance at ${td.location}, finishing on ${winner.points} points from ${td.format.match(/\d+/)?.[0] ?? '9'} rounds to take clear first place.

The tournament reached its decisive moment in round ${keyGame.round}, when ${keyGame.white} faced ${keyGame.black} in what proved to be the day's most significant encounter. After ${keyGame.moves} moves of ${keyGame.opening ?? 'complex middlegame play'}, the game concluded ${keyGame.result}, effectively settling the standings heading into the final round.
...
With the ${td.name} in the books, attention turns to the next fixture on the Pawn Stars calendar. ${winner.title} ${winner.playerName} will enter that event as the form player to beat, though a tightly contested field guarantees nothing. One certainty: the organisation continues to raise the standard of competitive chess in Spain and beyond.`;
  ...
  return {
    ...
    suggestedTags: [
      ...
      runnerUp.playerName,
      'Match Reports',
      td.location.split(',')[0] ?? td.location,
    ],
...
export class ArticleGenerationService {
  ...
  private readonly generationLog: GenerationLog[] = [];
  private readonly publishedDrafts: Array<{ newsPostId: string; slug: string; article: GeneratedArticle; createdAt: string }> = [];
  ...
  private getTournamentData(tournamentId: string): TournamentData | null {
    return MOCK_TOURNAMENTS[tournamentId] ?? null;
  }

  async generateArticle(request: ArticleGenerationRequest): Promise<GeneratedArticle> {
    const td = this.getTournamentData(request.tournamentId);
    if (!td) {
      throw new Error(`Tournament not found: ${request.tournamentId}`);
    }
    ...
    if (!anthropic) {
      this.logger.warn('ANTHROPIC_API_KEY not set — returning mock fallback article');
...
Please generate the post-tournament article now. Return ONLY valid JSON, no markdown fences.`.trim();

    try {
      const response = await anthropic.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 1024,
        system: `You are a professional chess journalist writing for Pawn Stars, a competitive chess organisation.
...
Return your response as a single JSON object with exactly these fields:
{
  ...
  "category": "MATCH_REPORTS"
      ...
      try {
        parsed = JSON.parse(rawContent.text) as typeof parsed;
      } catch {
        // Claude occasionally wraps JSON in markdown fences despite instructions
        const match = rawContent.text.match(/```(?:json)?\s*([\s\S]+?)\s*```/);
        if (match?.[1]) {
      ...
      this.logger.error(`Article generation failed: ${message}`);
      this.generationLog.push({
        id: logId,
        tournamentId: td.id,
        tournamentName: td.name,
  ...
  publishDraft(article: GeneratedArticle, authorName = 'AI Journalist'): PublishDraftResponse {
    ...
    const newsPostId = `np-${Date.now()}`;
    const createdAt = new Date().toISOString();

    this.publishedDrafts.push({ newsPostId, slug, article, createdAt });
    this.logger.log(`Draft published: ${newsPostId} — "${article.headline}" by ${authorName}`);

    return { newsPostId, slug, status: 'draft', createdAt };
  }

  getGenerationLog(): GenerationLog[] {
    return [...this.generationLog].reverse();
  }

  getAvailableTournaments(): Array<{ id: string; name: string; status: string }> {
    return Object.values(MOCK_TOURNAMENTS).map((t) => ({
      id: t.id,
      name: t.name,
      status: 'COMPLETED',
    }));
